...
library(pacman)
p_load(tidyverse, waffle, ggsankey,lubridate,kableExtra)
```
...
```{r out.width='70%', fig.cap='Example of an analyst population', fig.align='center'}
data <- tribble(
  ~Classification, ~n,
  "Busy", "32", 
  "Confused", "19",
...
These are great for showing flow through a system. One serious drawback though is that the ggsankey package is rather hard to understand [@ggsankey].

```{r out.width='100%', fig.cap='After-work outcomes'}
read_csv("data/sankey.csv")  %>%
  make_long(Day, Evening, Bedtime) %>%
  ggplot(aes(x = x, 
             next_x = next_x, 
             node = node, 
             next_node = next_node,
             label = node)) +
  geom_sankey(flow.alpha = 0.3, fill = "#009FE3") +
  geom_sankey_label(size = 4, color = "black", fill="#EAD9AF", space=20) +
  theme_void() +
